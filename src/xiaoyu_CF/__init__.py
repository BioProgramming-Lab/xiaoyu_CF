...
def FSC_SSC_ploting(experiment, type = "scatter"):
    if type == "scatter":
        ScatterplotView(xchannel = "FSC 488/10-A", ychannel = "SSC 488/10-A", yscale = "linear", xscale = "linear", huefacet = 'sample').plot(experiment, alpha = 0.005)
    elif type == "density":
        DensityView(xchannel = "FSC 488/10-A", ychannel = "SSC 488/10-A", yscale = "linear", xscale = "linear").plot(experiment)
    return

# gate FSC-A ~ SSC-A
def gate_FSC_SSC(experiment, vertices, if_plot = False):
    gate = PolygonOp(name = "FSCA_SSCA", xchannel = "FSC 488/10-A", ychannel = "SSC 488/10-A", vertices = vertices)
...
def auto_gate_FSC_SSC(experiment, if_plot = False, keep = 0.6):
    dens_gate = DensityGateOp(name = "auto_FSCA_SSCA", xchannel = "FSC 488/10-A", ychannel = "SSC 488/10-A", xscale = "linear", yscale = "linear", keep = keep)
...
def FSC_A_H_ploting(experiment):
    ScatterplotView(xchannel = "FSC 488/10-A", ychannel = "FSC 488/10-H", yscale = "linear", xscale = "linear", huefacet = 'sample').plot(experiment, alpha = 0.005)
    return

# gate FSC-A ~ FSC-H
def gate_FSC_A_H(experiment, vertices, if_plot = False):
    gate = PolygonOp(name = "FSCA_FSCH", xchannel = "FSC 488/10-A", ychannel = "FSC 488/10-H", vertices = vertices)
    gatedDATA = gate.apply(experiment)
    if if_plot:
        gate.default_view(xscale = "linear", yscale = "linear", huefacet = 'sample').plot(gatedDATA, s = 10, alpha = 0.1)
...
def auto_gate_FSC_A_H(experiment, if_plot = False, keep = 0.9):
    dens_gate = DensityGateOp(name = "auto_FSCA_FSCH", xchannel = "FSC 488/10-A", ychannel = "FSC 488/10-H", xscale = "linear", yscale = "linear", keep = keep)
    dens_gate.estimate(experiment)
    if if_plot:
        dens_gate.default_view().plot(experiment)
...
def log_density_plot(experiment, xchannel = "FSC 488/10-A", ychannel = "FSC 488/10-H", huefacet = "", **kwargs):
    if huefacet != "":
        DensityView(xchannel = xchannel, ychannel = ychannel, xscale = "log", yscale = "log", huefacet = huefacet).plot(experiment, **kwargs)
    else:
        DensityView(xchannel = xchannel, ychannel = ychannel, xscale = "log", yscale = "log").plot(experiment, **kwargs)
    return
...
def channel_histogram(experiment, channel = "Alexa 647-A", huefacet = "sample", bins = 200, title = "", **kwargs):
    if title == "":
        if huefacet == "conc":
            HistogramView(channel = channel, scale = "log", huefacet = huefacet, huescale = "log").plot(experiment, num_bins = bins, alpha = 0.9, density = True, **kwargs)
        else:
            HistogramView(channel = channel, scale = "log", huefacet = huefacet).plot(experiment, num_bins = bins, density = True, **kwargs)
        return
    else:
        if huefacet == "conc":
            HistogramView(channel = channel, scale = "log", huefacet = huefacet, huescale = "log").plot(experiment, num_bins = bins, alpha = 0.9, density = True, title = title, **kwargs)
        else:
            HistogramView(channel = channel, scale = "log", huefacet = huefacet).plot(experiment, num_bins = bins, density = True, title = title, **kwargs)
        return
...
def polygon_gate(experiment, xchannel = "FSC 488/10-A", ychannel = "SSC 488/10-A", xscale = "linear", yscale = "linear", cmap='magma_r', vertices = [], if_plot = False):
    ...
    if vertices != []:
        ...
        gate = PolygonOp(name = f"polygon_gate_{polygon_gate_number}", xchannel = xchannel, ychannel = ychannel, vertices = draggable_polygon.points)
        gatedDATA = gate.apply(experiment)
        polygon_gate_number += 1
        if if_plot:
            gate.default_view(xscale = xscale, yscale = yscale).plot(gatedDATA, s = 10, alpha = 0.1)
